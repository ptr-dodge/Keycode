...
var renderer, toDispose, photoTexture, photoMaterial, photoBitmap, photoWidth, photoHeight;
var cv = [ ];
var manualDepths = { };

function bitting(mfgr, pins) {
    var spacings = [ ], depths = [ ], width = '';
    if(mfgr === 'kw') {
        spacings = [ 0.247, 0.397, 0.547, 0.697, 0.847, 0.997 ];
        depths = [ 'unused', 0.329, 0.306, 0.283, 0.260, 0.237, 0.214, 0.191 ];
        width = '0.335';
    } else if(mfgr === 'sc') {
        spacings = [ 0.231, 0.3872, 0.5434, 0.6996, 0.8558, 1.012 ];
        depths = [ 0.335, 0.320, 0.305, 0.290, 0.275, 0.260, 0.245, 0.230, 0.215, 0.200 ];
        width = '0.343';
    }

    spacings = spacings.slice(0, pins);

    $('#bitting_spacings').val((spacings.map(function(x) { return x.toFixed(4) })).join(', '));
    $('#bitting_width').val(width);
    $('#bitting_depths').val(depths.map(function(x) {
...
function loadImage(uri, sample) {
    ...
    if(sample) {
        modalMessage('Loaded sample image. The bitting has been set to KW1.<br><br>This image has already been aligned to the red and blue guides. If you select the Align tab, then you\'ll see the correct code.<br><br>When you load your own image, you\'ll need to align it to the guides by hand. Try moving and rotating the sample image. Watch how the green detected cuts and the code change, to learn how the tools work.<br><br>If the detected edges of the cuts are wrong, then click or tap to place manually, again to clear.');
    } else {
...
function loadSampleImage() {
    ...
    xfrm.hg = [0.5018518300223738, 0.03280486662499481, 0.008149327540922235, -0.019599621583268556, 0.8399745004999475, 0.20262731136053902, 1.8410231821756825e-18, 3.6041222243318587e-19, 0.0017423903114348802];
    
    xfrm.ihg = [0.003369933236284373, -0.00013161138854292278, -0.00045605621394176343, 0.00007863264438711157, 0.0020134029791094104, -0.23451188452733207, -3.5210555230054996e-18, -2.1255039632640669e-19, 0.9721052074142724];
...
function normalizeHomography() {
    [ 'hg', 'ihg' ].forEach(function (p) {
        ...
        sum = Math.sqrt(sum);
        xfrm[p] = xfrm[p].map(function(x) { return x/sum; });
    });
}
...
function solveForHomography() {
    ...
    function solve(ins, outs, hg) {
        ...
        //
        for(i = 0; i < 8; i += 2) {
            var pin = ins[i/2], pout = outs[i/2];

            A[i  ] = [ pin.x, pin.y, 1,     0,     0, 0, -pout.x*pin.x, -pout.x*pin.y, -pout.x ];
            A[i+1] = [ 0,     0,     0, pin.x, pin.y, 1, -pout.y*pin.x, -pout.y*pin.y, -pout.y ];
...
function resetHomography() {
    var img = $('#photo')[0], r = img.naturalWidth / img.naturalHeight,
        mx = xfrm.vw / 2, my = xfrm.vh / 2;
    if(r > mx/my) {
        my = mx/r;
    } else {
        mx = my*r;
...
function Vector2(x, y) {
    return {
        ...
        afterHomography: function(hg) {
            ...
            return Vector2((hg[0]*x + hg[1]*y + hg[2])/denom,
                           (hg[3]*x + hg[4]*y + hg[5])/denom);
        },
        hg  : function() { return this.afterHomography(xfrm.hg); },
        ihg : function() { return this.afterHomography(xfrm.ihg); },
        toString: function() {
...
function parseBittings() {
    ...
    b.spacings = $('#bitting_spacings').val().split(',').map(function(x) { return parseFloat(x) });
    b.depths = $('#bitting_depths').val().split(',').map(function(x) { return parseFloat(x) });
...
function plotColorChannels(digit) {
    ...
    var canvas = $('#graph')[0], ctx = canvas.getContext('2d');
    ...
    var xmax = sp.x[sp.x.length-1], xmin = sp.x[0];

    mapX = function(x) { return (x - xmin)*700/(xmax - xmin); }
    mapY = function(y) { return (370 - 1.3*y); }
    ...
    for(var i = 0; i < b.depths.length; i++) {
        ...
        var text = i.toString(), tw = ctx.measureText(text).width;
    ...
    [ 'red', 'green', 'blue', 'disc' ].forEach(function(trace) {
        ...
        var d = sp[trace], dx = sp.x;

        for(var i = 0; i < sp.n; i++) {
            var x = mapX(dx[i]), y = mapY(d[i]);
            if(i == 0) {
                ctx.moveTo(x, y);
            } else {
                ctx.lineTo(x, y);
...
/*    var img = $('#photo')[0], canvas = $('#photoc')[0];
    ...
    b.spacings.slice(0).forEach(function(ysp) {
        ...
        var step = 0.001, beyond = 0.080, slop = 0.015;
        for(var x = -b.width/2 + mind - slop; x < b.width/2 + beyond; x += step) {
            var tr = 0, tg = 0, tb = 0;
            [ -0.002, -0.001, 0, 0.001, 0.002 ].forEach(function(dy) {
                var pt = Vector2(x*xfrm.pixelsPerInch, (y+dy)*xfrm.pixelsPerInch), bpt = pt.ihg();
...
/*                pix[i+0] = 0;
        ...
        var disc = [ ], rightN = 0, rightR = 0, rightG = 0, rightB = 0;
        for(i = pr.length - 1; i >= 1; i--) {
            ...
            var rar = rightR/rightN, rag = rightG/rightN, rab = rightB/rightN,
                dr = rar - pr[i-1], dg = rag - pr[i-1], db = rab - pr[i-1],
        ...
        var hpf = [ 0.4, 0.7, 1, -1, -0.7, -0.4 ], hpfd = [ 0, 0, 0, ];
        ...
        var maxNoise = 6, discmaxi = disc.length - 1, margin = ((beyond - slop)/step)|0;
...
//        disc = [ ]; hpfd.forEach(function(v) { disc.push(v+128); });
        ...
        var xedge = px[discmaxi], code = '?', dxedge = 100;
...
function makeKeyCodeHtml() {
    ...
    var table = $('#code'), tra = $('<tr>'), trb = $('<tr>'), trc = $('<tr>'), i = 0;
    tra.append($('<th>').text('code'))
    trb.append($('<th>').text('error'))
    trc.append($('<th>'));
    cv.forEach(function(sp) {
        var axe = Math.abs(sp.xerror), code = sp.code, xe = sp.xerror.toFixed(3), cl;
...
function render() {
    ...
    var A = [ ax,           0,              0,  0,
              0,            ay,             0,  0,
              0,            0,              ax, 0,
              -xfrm.vdx*ax, xfrm.vdy*ay,    0,  1 ];

    for(var i = 0; i < 16; i++) cpme[i] = A[i];

    var scene = pool.s, lgs = pool.g.l;
    ...
    function line(x0, y0, x1, y1, w, color) {
        ...
        var lg = lgs[color], i = lg.vertices.length;
    ...
    inchLine( b.width/2, b.shoulder,  b.width/2 + b.guide, b.shoulder, 2, 'b');
    inchLine( b.width/2 + b.guide, b.shoulder,  b.width/2 + b.guide, b.shoulder+0.2, 2, 'k');
    inchLine(-b.width/2 - b.guide, b.shoulder, -b.width/2 - b.guide, b.shoulder+0.2, 2, 'k');

    inchLine(-b.width/2, b.shoulder, -b.width/2, b.shoulder - b.guideLength, 2, 'r');
    inchLine( b.width/2, b.shoulder,  b.width/2, b.shoulder - b.guideLength + 0.2, 2, 'r');
    inchLine( b.width/2,        b.shoulder - b.guideLength + 0.2,
              b.width/2 - 0.03, b.shoulder - b.guideLength + 0.1, 2, 'k');

    b.spacings.forEach(function(y) {
        if(isNaN(y)) return;
        inchLine(-b.width/2, b.shoulder-y, b.width/2 + 0.5, b.shoulder-y, 1, 'g');
    });

    b.depths.forEach(function(x) {
        if(isNaN(x)) return;
        inchLine(-b.width/2 + x, b.shoulder - b.minsp + 0.1,
                 -b.width/2 + x, b.shoulder - b.maxsp - 0.1, 1, 'g');
    });

    cv.forEach(function(sp) {
        var x = -b.width/2 + sp.xedge;
        inchLine(x, sp.y - 0.030, x, sp.y + 0.030, 1, 'y');
        if(sp.manual) {
            var o = Vector2(x - 0.040, sp.y + 0.020), l = 0.030, ly = 0.020, r = 0.013;
...
function onePointMoveInteraction(x, y, xp, yp, xd, yd) {
    ...
    switch($('input[name=manip_mouse]:checked').val()) {
        ...
        case 'rotate_scale':
            ...
            [ 'pa', 'pb', 'pc', 'pd' ].forEach(function(p) {
                xfrm[p] = xfrm[p].minus(c).rotatedAboutOrigin(dtheta).scaledBy(a).plus(c);
            });
            break;
        ...
        case 'free_xfrm':
            ...
            var p = [ 'pa', 'pb', 'pc', 'pd' ]
            p.sort(function(a, b) { return xfrm[a].y - xfrm[b].y });
            if(xfrm[p[0]].x < xfrm[p[1]].x) {
                var t = p[0]; p[0] = p[1]; p[1] = t;
            }
            if(xfrm[p[2]].x < xfrm[p[3]].x) {
                var t = p[2]; p[2] = p[3]; p[3] = t;
            }
            p = p[i];
...
function mouseMove(ev) {
    ...
    onePointMoveInteraction(ev.offsetX, ev.offsetY, xfrm.mx, xfrm.my, xfrm.mdx, xfrm.mdy);
...
function touchMove(ev) {
    ...
    if(ev.touches.length == 1) {
        ...
        onePointMoveInteraction(x, y, xfrm.t[id].x, xfrm.t[id].y, xfrm.td[id].x, xfrm.td[id].y);
...
function mirror() {
    var t;
    t = xfrm.pa; xfrm.pa = xfrm.pd; xfrm.pd = t;
    t = xfrm.pb; xfrm.pb = xfrm.pc; xfrm.pc = t;
...
function main() {
    ...
    $('#list_of_tabs').children().each(function() {
        var id = this.id, tab = 'content_' + id.substr(4);
        this.onclick = function() {
            if((id === 'tab_align' || id === 'tab_code') && !photoMaterial) {
                modalMessage('Need to load photograph of key before aligning and getting code.');
                return;
            }
            var b = parseBittings();
            ...
            $('#content_of_tabs').children().each(function() {
                if(this.id === tab) {
        ...
        if(first) this.onclick();
        first = false;
    });
//    bitting('sc', 5);

    // do hover by hand to avoid leftover hover on touch displays
    var fh = $('button').add($('div#list_of_tabs a'));
    fh.mouseenter(function(ev) {
        var now = (new Date()).getTime();
        if(now < ev.target.blockMouseEnterUntil) return;

        $(ev.target).addClass('fakehover');
    });
    fh.mouseleave(function(ev) {
        $(ev.target).removeClass('fakehover');
    });
    ...
    for(var i = 0; i < fh.length; i++) {
        fh[i].ontouchstart = f;
        fh[i].ontouchend = f;
        fh[i].addEventListener('click', f);
        fh[i].blockMouseEnterUntil = 0;
    }
